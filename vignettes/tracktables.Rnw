...
Further configuration of the report can be achieved through the use of the \texttt{colourBy} and \texttt{bwScale} arguments passed to the \Rfunction{maketracktables()} function. The \texttt{colourBy} argument accepts a character argument corresponding to the metadata column by which samples will be coloured in IGV. The \texttt{bwScale} argument defines how bigwigs will be scaled in IGV. \texttt{bwScale} accepts either a character argument corresponding to supported scaling methods (currently only "autoscale") or a list of numeric values corresponding to the minimums and maximums of the data ranges for BigWig files displayed in IGV.
<<maketracktable_WithColoursAndScales, eval=FALSE>>=
bwRanges <- list(
            c(0,20),c(0,2),c(0,10),c(0,2)
            )
  HTMLreport <- maketracktable(FileSheet,SampleSheet,
                               "IGVExample.html",
                               basedirectory=getwd(),
                               "mm9",colourBy="Antibody",bwScale=bwRanges)
@
